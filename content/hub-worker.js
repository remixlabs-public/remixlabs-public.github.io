...
const MQTT = __webpack_require__(1);
const NanoID = __webpack_require__(3);

const brokerStatusTopic = "/brokerStatus";

function setup() {
    self.onmessage = onmessage;
    self.state = { paho: null,
                   config: {},
                   buffer: [],
                   connected: false,
                   nextStatus: "virgin",
    ...
    self.state.localPubTopics.set(brokerStatusTopic, true);
    self.state.localSubTopics.set(brokerStatusTopic, true);
...
function on_msg_configure(msg) {
    self.state.config.user = msg.user;
    self.state.config.token = msg.token;
    ...
    // console.log("messaging hub: using MQTT ID of " + clientId);
    self.state.paho = new MQTT.Client(ws_url, clientId);
    self.state.paho.onMessageArrived = on_paho_messageArrived;
    let first = true;
    self.state.paho.onConnected = ((reconnected,uri) => {
        self.state.connected = true;
        self.state.nextStatus = reconnected ? "reconnected" : "connected";
        logStatus();
        if (first)
            self.postMessage({ error: null });
        first = false;
        while (self.state.buffer.length > 0) {
            let f = self.state.buffer.shift();
            try {
                f();
            } catch (e) {
                console.error(e);
        ...
        self.state.subscriptions.forEach((subs, topic) => {
            broker_subscribe(topic, undefined, undefined, true)
        });
        if (are_all_subscriptions_made()) {
            deliverStatus(self.state.nextStatus);
        }
    });
    self.state.paho.onConnectionLost = (eobject => {
        console.error("messaging hub: connection lost, " + "code " + eobject.errorCode + " " + eobject.errorMessage);
        self.state.connected = false;
        self.state.nextStatus = "disconnected";
        self.state.subStatus = new Map();
    ...
    self.state.paho.connect(opts);
}

function logStatus() {
    console.info("messaging hub: " + (self.state.connected ? "connected to " : "disconnected from ") + self.state.paho.host + ":" + self.state.paho.port)
}

function on_msg_newChannel(port, msg) {
    let channel = new MessageChannel();
    channel.port1.onmessage = (ev => chan_onmessage(channel.port1, ev));
    port.postMessage( { error:null, port:channel.port2 }, [ channel.port2 ] )
}

function chan_onmessage(port, ev) {
    let msg = ev.data;
    switch (msg._rmx_type) {
...
function on_chan_setLocalPubTopic(port, msg) {
    self.state.localPubTopics.set(msg.topic, true);
    let resp = { error: null, reqId: msg.reqId };
    port.postMessage(resp);
}

function on_chan_setLocalSubTopic(port, msg) {
    self.state.localSubTopics.set(msg.topic, true);
    let resp = { error: null, reqId: msg.reqId };
    port.postMessage(resp);
}

function when_connected(f) {
    if (self.state.connected) {
        f()
    } else {
        self.state.buffer.push(f);
    }
}

function on_chan_publish(port, msg) {
    ...
    if (msg.retained) {
        self.state.retained.set(msg.topic, msg)
    };
    deliver(msg);   // local delivery
    if (self.state.paho === null || msg.localOnly || self.state.localPubTopics.has(msg.topic)) {
        let resp = { error:null, reqId:msg.reqId };
        port.postMessage(resp);
        return
    };
    let header =
        ...
        ",_rmx_route=" + self.state.paho.clientId + "\n";
    ...
    when_connected(() => {
        self.state.paho.publish(msg.topic, buf.buffer, 0, msg.retained);
    });
    let resp = { error:null, reqId:msg.reqId };
    port.postMessage(resp);
}

function are_all_subscriptions_made() {
    let ok = true;
    self.state.subscriptions.forEach((_, topic) => {
        let status = self.state.subStatus.get(topic);
        if (!status) ok = false;   // status == false || status == undefined
    });
    return ok;
}

function broker_subscribe(topic, port, reqId, autoRetry) {
    if (!self.state.connected) return;
    if (self.state.subStatus.has(topic)) return;
    self.state.subStatus.set(topic, false);
    let opts =
        { qos: 0,
          onSuccess: (ctx => {
              console.log("messaging hub: subscribed to " + topic);
              self.state.subStatus.set(topic, true);
    ...
    self.state.paho.subscribe(topic, opts);
}

function on_chan_subscribe(port, msg) {
    let sub = { port:port, subId: msg.subId };
    if (self.state.subscriptions.has(msg.topic)) {
        let list = self.state.subscriptions.get(msg.topic);
        list.push(sub);
        port.postMessage({ error:null, reqId:msg.reqId });
        if (self.state.retained.has(msg.topic)) {
            deliver(self.state.retained.get(msg.topic))
        };
        return
    };
    let list = new Array();
    list.push(sub);
    self.state.subscriptions.set(msg.topic, list);
    if (self.state.paho === null || self.state.localSubTopics.has(msg.topic)) {
        let resp = { error:null, reqId: msg.reqId };
        port.postMessage(resp);
        if (self.state.retained.has(msg.topic)) {
            deliver(self.state.retained.get(msg.topic))
    ...
    when_connected(() => {
        broker_subscribe(msg.topic, port, msg.reqId, false);
    })
}

function on_chan_unsubscribe(port, msg) {
    if (self.state.subscriptions.has(msg.topic)) {
        let list = self.state.subscriptions.get(msg.topic);
        let newList = list.filter(sub => sub.subId != msg.subId);
        if (newList.length > 0) {
            self.state.subscriptions.set(msg.topic, newList);
            let resp = { error:null, reqId: msg.reqId };
            port.postMessage(resp)
        } else {
            self.state.subscriptions.delete(msg.topic);
            if (self.state.paho === null || self.state.localSubTopics.has(msg.topic)) {
                ...
                if (self.state.connected) {
                    self.state.paho.unsubscribe(msg.topic, opts);
                };
            }
        };
        let eof =
...
function on_paho_messageArrived(mqtt_msg) {
    ...
    for (let prop of props) {
        ...
        case "_rmx_route":
            let route = p[1].split(";");
            if (route.includes(self.state.paho.clientId)) {
...
function deliverStatus(status) {
    if (status == self.state.lastStatus) return;
    self.state.lastStatus = status;
    ...
    self.state.retained.set(topic, msg);
